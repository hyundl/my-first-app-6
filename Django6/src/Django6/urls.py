...
# 해당 파일이 view 함수를 알아야되므로 from ~ import 로 함수 추가
...
urlpatterns = [
    ...
    # URL추가 http://127.0.0.1:8000 주소로 접근시 vote 어플리케이션 내의 index 함수 호출
    # path(문자열, 함수명) : 웹 클라이언트가 문자열에 해당하는 URL로 접근 시 매칭된 함수를 호출함
    #
    ...
    # http://127.0.0.1:8000/vote/로 시작하는 URL들을 vote폴더 안에 있는 urls 파이썬 파일로 처리하도록 매칭
    # include 하면서 그룹이름을 정하고싶은 경우
