...
# 장고에서 제공하는 로그아웃 기능을 사용한 뒤 돌아올 페이지 설정
# reverse() : 해당 함수를 호출했을 때 즉시 urls 파일을 탐색해 url주소를 찾음
# reverse_lazy() : 해당 함수를 호출했을 때 웹서버 실행이 준비되면 urls 파일을 탐색해 url주소를 찾음(헷갈리면 이거쓰기)
LOGOUT_REDIRECT_URL = reverse_lazy('vote:index')

# 로그인이 요구되는 기능을 비로그인 상태로 접근할 때 로그인할 수 있는 페이지 설정
...
# 클라이언트 id
SOCIAL_AUTH_GOOGLE_OAUTH2_KEY = '521835322399-d1ifrrc2t35mvft1nq7r3gn6odaebv9s.apps.googleusercontent.com'
# 클라이언트 보안비밀
SOCIAL_AUTH_GOOGLE_OAUTH2_SECRET = 'Cl5eIrobQIuPUzsN1Jz3mAw8'
...
AUTHENTICATION_BACKENDS=(
    ...
    # 구글 로그인과 프로젝트의 User 모델클래스와 매칭할 때 사용
